      ...
      any: (node) => {
        ...
        for (let nodeIndex = 0; nodeIndex < node.content.length; nodeIndex++) {
          ...
          if (
            ...
            while (
              (index = childText.indexOf(comment.textForComment, index)) !== -1
            ) {
    ...
    const comments = [
      ...
      ol({ order: 1 })(
        ...unfoundInlineComments.map((item) =>
          li([
            p(commentedText(item.textForComment ?? "", item.inlineCommentId)),
          ]),
        ),
      ),
    ];
...
function removeEmptyProperties(adf: JSONDocNode) {
  return traverse(adf, {
    any: (node, _parent) => {
      if (
        node.content &&
        node.content.filter((m) => !(m === undefined || m === null)).length ===
          0
      ) {
        delete node.content;
      }
      ...
      try {
        if (
          node.marks &&
          node.marks.filter((m) => !(m === undefined || m === null)).length ===
            0
        ) {
          delete node.marks;
        }
