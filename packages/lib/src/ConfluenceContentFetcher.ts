...
export class ConfluenceContentFetcher {
    ...
    for (let attempt = 0; attempt <= retryAttempts; attempt++) {
      try {
        ...
        if (attempt < retryAttempts) {
          console.warn(
            `Failed to fetch page ${pageId} (attempt ${attempt + 1}/${
              retryAttempts + 1
            }):`,
            lastError.message,
          );
  ...
  async fetchChildren(
    ...
    for (let attempt = 0; attempt <= retryAttempts; attempt++) {
      try {
        ...
        if (
          !parentPageWithChildren.children?.page?.results ||
          parentPageWithChildren.children.page.results.length === 0
        ) {
          return [];
        }

        const childIds = parentPageWithChildren.children.page.results.map(
          (child: any) => child.id,
        );
        ...
        if (
          childIds.length >= 25 &&
          (parentPageWithChildren.children.page as any)._links?.next
        ) {
          console.warn(
            `Page ${parentId} may have more than ${childIds.length} children. Some may be missing due to API pagination limits.`,
          );
        }
        ...
        if (attempt < retryAttempts) {
          console.warn(
            `Failed to fetch children for page ${parentId} (attempt ${
              attempt + 1
            }/${retryAttempts + 1}):`,
            lastError.message,
          );
    ...
    throw new Error(
      `Failed to fetch children for page ${parentId} after ${
        retryAttempts + 1
      } attempts: ${lastError?.message}`,
    );
  }
  ...
  async fetchPageTree(
    ...
    const pagesToProcess: { id: string; depth: number }[] = [
      { id: rootPageId, depth: 0 },
    ];
