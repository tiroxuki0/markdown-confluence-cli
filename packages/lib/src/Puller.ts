...
export class Puller {
  ...
  async pullPageTree(
    ...
    try {
      ...
      const allPages = await this.contentFetcher.fetchPageTree(
        rootPageId,
        options.maxDepth || 10,
      ); // Use configurable max depth with fallback
      ...
      if (rootPage) {
        ...
        const rootFileResult = await this.fileGenerator.generateFile(
          rootPage,
          rootMarkdown,
          options,
          pageHierarchy,
  ...
  private convertPageToMarkdown(page: ConfluencePageData): string {
    ...
    if (page.body?.atlas_doc_format?.value) {
      try {
        ...
        markdown += this.removeDuplicateTitleHeadings(
          contentMarkdown,
          page.title,
        );
      } catch (error) {
   ...
   */
  private removeDuplicateTitleHeadings(
    markdown: string,
    pageTitle: string,
  ): string {
